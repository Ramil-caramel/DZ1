...
std::string pre_obrabotka(std::string&);// добавляет ( ) в начало и конец, а также переписывает отрицательные числа

std::string Polish_Notation(std::string&);// создает польскую нотацию

std::string post_obrabotka(std::string&);// доделывает польскую нотацию так как там встреччаются дополнительные лишние пробелы, а я не хочу усложнять алгоритм
